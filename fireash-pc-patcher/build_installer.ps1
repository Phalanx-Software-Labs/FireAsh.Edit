...
$innoIscc = $null
foreach ($innoCandidate in @(
        "${env:ProgramFiles(x86)}\Inno Setup 6\ISCC.exe",
        "${env:ProgramFiles}\Inno Setup 6\ISCC.exe"
    )) {
    if (-not [string]::IsNullOrWhiteSpace($innoCandidate) -and (Test-Path -LiteralPath $innoCandidate)) {
        $innoIscc = [string]$innoCandidate
        break
    }
}

if ([string]::IsNullOrWhiteSpace($innoIscc)) {
...
Write-Host "==> Inno Setup: $innoIscc" -ForegroundColor Cyan
$iss = Join-Path $Here "installer\FireAsh.Edit.iss"
$innoArgList = '"' + $iss + '"'
$innoProc = Start-Process -FilePath $innoIscc -ArgumentList $innoArgList -Wait -PassThru -NoNewWindow
if ($innoProc.ExitCode -ne 0) { exit $innoProc.ExitCode }
